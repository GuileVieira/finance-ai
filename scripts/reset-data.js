...
async function main() {
  try {
    ...
    const { db } = await import('../lib/db/connection.js');
    const { uploads, transactions, accounts, companies } = await import('../lib/db/schema.js');
    ...
    console.log(`📅 Modo ${flags.recent ? 'recente (1 hora)' : 'completo'}`);
